...
public class ClusterController {
	...
	/**
	 * Start the cluster described by <code>clusterSpec</code> and block until
	 * the cluster is available. It is not guaranteed that the service running
	 * on the cluster has started when this method returns.
	 * 
	 * @param clusterSpec
	 * @return an object representing the running cluster
	 * @throws IOException
	 *             if there is a problem while starting the cluster. The cluster
	...
	public Cluster bootstrapCluster(ClusterSpec clusterSpec)
		...
		BootstrapClusterAction bootstrapper = new BootstrapClusterAction(
				getCompute(), handlerMapFactory.create());
	...
	public Cluster configureServices(ClusterSpec clusterSpec, Cluster cluster,
			...
			throws IOException, InterruptedException {
		ConfigureServicesAction configurer = new ConfigureServicesAction(
				getCompute(), handlerMapFactory.create(), targetRoles,
				targetInstanceIds);
	...
	public Cluster startServices(ClusterSpec clusterSpec, Cluster cluster,
		...
		StartServicesAction starter = new StartServicesAction(getCompute(),
				handlerMapFactory.create(), targetRoles, targetInstanceIds);
	 ...
	 *             may or may not have been stopped.
	 * @throws InterruptedException
	 *             if the thread is interrupted.
	 */
	...
	public Map<? extends NodeMetadata, ExecResponse> runScriptOnNodesMatching(
			ClusterSpec spec, Predicate<NodeMetadata> condition,
			Statement statement) throws IOException, RunScriptOnNodesException {
		return runScriptOnNodesMatching(spec, condition, statement, null);
	}

	public Map<? extends NodeMetadata, ExecResponse> runScriptOnNodesMatching(
			ClusterSpec spec, Predicate<NodeMetadata> condition,
			Statement statement, RunScriptOptions options) throws IOException,
		...
		condition = Predicates.and(runningInGroup(spec.getClusterName()),
				condition);
	...
	public Set<? extends NodeMetadata> getNodes(ClusterSpec clusterSpec)
		...
		return computeService
				.listNodesDetailsMatching(runningInGroup(clusterSpec
						.getClusterName()));
	}
	...
	public Set<Cluster.Instance> getInstances(ClusterSpec spec,
			ClusterStateStore stateStore) throws IOException,
			InterruptedException {
	...
	private Cluster.Instance toInstance(NodeMetadata metadata, Cluster cluster,
			ClusterSpec spec) {
		...
		try {
			if (cluster != null) {
				roles = cluster.getInstanceMatching(withIds(metadata.getId()))
						.getRoles();
			}
		} catch (NoSuchElementException e) {
		}

		return new Cluster.Instance(credentials, roles, Iterables.getFirst(
				metadata.getPublicAddresses(), null), Iterables.getFirst(
				metadata.getPrivateAddresses(), null), metadata.getId(),
				metadata);
	}
	...
	public Cluster runExperiment(ExperimentPlan experimentPlan,
			...
			InterruptedException {
		RunExperimentAction run = new RunExperimentAction(getCompute(),
				handlerMapFactory.create(), targetRoles, targetInstanceIds);
		Cluster toReturn = run.execute(clusterSpec, cluster);
		return toReturn;
	}
