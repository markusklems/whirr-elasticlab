...
public class ClusterSpec {
	...
	public enum Property {
		...
		MAX_STARTUP_RETRIES(Integer.class, false,
				"The number of retries in case of insufficient "
		...
		STATE_STORE_CONTAINER(String.class, false,
				"Container where to store state. "
						+ "Valid only for the blob state store."),

		STATE_STORE_BLOB(
				String.class,
				false,
				"Blob name for state storage. "
						+ "Valid only for the blob state store. Defaults to whirr-<cluster-name>"),

		AWS_EC2_SPOT_PRICE(Float.class, false,
				"Spot instance price (aws-ec2 specific option)"),
		...
		FIREWALL_RULES_ROLE(
				String.class,
				true,
				"A comma-separated list of port"
	...
	public static ClusterSpec withTemporaryKeys(Configuration conf)
		...
		if (!conf.containsKey(Property.PRIVATE_KEY_FILE.getConfigName())) {
			Map<String, File> keys = org.apache.whirr.util.KeyPair
					.generateTemporaryFiles();

			LoggerFactory.getLogger(ClusterSpec.class).debug(
					"ssh keys: " + keys.toString());

			conf.addProperty(Property.PRIVATE_KEY_FILE.getConfigName(), keys
					.get("private").getAbsolutePath());
			conf.addProperty(Property.PUBLIC_KEY_FILE.getConfigName(), keys
					.get("public").getAbsolutePath());
		}
	...
	private String ycsbExperimentAction;
	private String ycsbDb;
	private String ycsbWorkloadFile;
	...
	public ClusterSpec(Configuration userConfig, boolean loadDefaults)
		...
		Pattern firewallRuleKeyPattern = Pattern.compile("^".concat(Pattern
				.quote(firewallPrefix).concat("(?:\\.(.+))?$")));
		for (String key : Iterators.<String> toArray(config.getKeys(),
				String.class)) {
		...
		setYcsbExperimentAction(getString(Property.YCSB_EXPERIMENT_ACTION));
		setYcsbDb(getString(Property.YCSB_DB));
		setYcsbWorkloadFile(getString(Property.YCSB_WORKLOAD_FILE));
	}
	...
	private Configuration composeWithDefaults(Configuration userConfig)
		...
		composed.addConfiguration(new PropertiesConfiguration(getClass()
				.getClassLoader().getResource(DEFAULT_PROPERTIES)));
		return composed;
	}
	...
	protected void checkAndSetKeyPair() throws ConfigurationException {
		...
		try {
			...
			publicKeyPath = (publicKeyPath == null && privateKeyPath != null) ? privateKeyPath
					+ ".pub"
					: publicKeyPath;
			if (privateKeyPath != null && publicKeyPath != null) {
				pairRepresentation = "(" + privateKeyPath + ", "
						+ publicKeyPath + ")";
				KeyPair pair = KeyPair.load(new JSch(), privateKeyPath,
						publicKeyPath);
				if (pair.isEncrypted()) {
				...
				if (!sameKeyPair(new File(privateKeyPath), new File(
						publicKeyPath))) {
	...
	public InstanceTemplate getInstanceTemplate(String... roles) {
		return getInstanceTemplate(Sets.newLinkedHashSet(Lists
				.newArrayList(roles)));
	}
	...
	public String getYcsbExperimentAction() {
		return ycsbExperimentAction;
	}
	public String getYcsbDb() {
		return ycsbDb;
	}
	public String getYcsbWorkloadFile() {
		return ycsbWorkloadFile;
	}
	...
	public void setStateStore(String type) {
		if (type != null) {
			checkArgument(Sets.newHashSet("local", "blob", "memory", "none")
					.contains(type),
	...
	private void checkContainerName(String name) {
		if (name != null) {
			checkArgument((new DnsNameValidator(3, 63) {
			}).apply(name));
		}
	}
	...
	public void setTerminateAllOnLaunchFailure(
			boolean terminateAllOnLaunchFailure) {
	...
	private void checkPublicKey(String publicKey) {
		 ...
		 */
		checkArgument(
				checkNotNull(publicKey, "publicKey").startsWith(
	...
	private void checkPrivateKey(String privateKey) {
		checkArgument(
				checkNotNull(privateKey, "privateKey").startsWith(
	...
	public void setClusterUser(String user) {
		checkArgument(user == null || !user.equals("root"),
	...
	public void setYcsbExperimentAction(String ycsbExperimentAction) {
		this.ycsbExperimentAction = ycsbExperimentAction;
	}

	public void setYcsbDb(String ycsbDb) {
		this.ycsbDb = ycsbDb;
	}

	public void setYcsbWorkloadFile(String ycsbWorkloadFile) {
		this.ycsbWorkloadFile = ycsbWorkloadFile;
	}
	...
	public boolean equals(Object o) {
		if (o instanceof ClusterSpec) {
			...
			return Objects.equal(getInstanceTemplates(),
					that.getInstanceTemplates())
					&& Objects.equal(getMaxStartupRetries(),
							that.getMaxStartupRetries())
					...
					&& Objects.equal(getBlobStoreProvider(),
							that.getBlobStoreProvider())
					&& Objects.equal(getBlobStoreIdentity(),
							that.getBlobStoreIdentity())
					...
					&& Objects.equal(getBootstrapUser(),
							that.getBootstrapUser())
					...
					&& Objects.equal(getHardwareMinRam(),
							that.getHardwareMinRam())
					...
					&& Objects.equal(getStateStoreBlob(),
							that.getStateStoreBlob())
					&& Objects.equal(getAwsEc2SpotPrice(),
							that.getAwsEc2SpotPrice());
		}
		return false;
	}

	public int hashCode() {
		return Objects.hashCode(getInstanceTemplates(), getMaxStartupRetries(),
				getProvider(), getIdentity(), getCredential(),
				getBlobStoreProvider(), getBlobStoreIdentity(),
				getBlobStoreCredential(), getBlobStoreCacheContainer(),
				getClusterName(), getServiceName(), getClusterUser(),
				getBootstrapUser(), getPublicKey(), getPrivateKey(),
				getImageId(), getHardwareId(), getHardwareMinRam(),
				getLocationId(), getBlobStoreLocationId(), getClientCidrs(),
				getVersion(), getRunUrlBase(), getStateStore(),
				getStateStoreBlob(), getStateStoreContainer(),
				getAwsEc2SpotPrice());
	}
	...
	public String toString() {
		return Objects
				...
				.add("terminateAllOnLauchFailure",
						isTerminateAllOnLaunchFailure()).toString();
	}
}
